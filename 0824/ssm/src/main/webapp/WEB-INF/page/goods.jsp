...
<form class="layui-form" action="" id="goodsForm" lay-filter="goodsForm" style="display: none;margin-right: 20px">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <select name="categoryId" id="category" lay-filter="categoryId">
                <option value="">请选择</option>
            </select>
        </div>
    </div>
...
<script>
    layui.use(['table','layer','form','laydate'], function(){
        ...
        table.render({
            ...
            ,url: '${pageContext.request.contextPath}/Goods/goodsList'//请求地址
        ...
        table.on("tool(goodsTable)",function (obj){
            ...
            switch (obj.event){
                case "goodsEdit":
                    $("#goodsForm")[0].reset();
                    $("#actionUrl").val("${pageContext.request.contextPath}/Goods/update");
        ...
        /*加载商品产地的下拉框*/
/*        $.get(
            "${pageContext.request.contextPath}/Goods/addressList",
            function (res){
                if(res.code==0){
            ...
            function (res){
                if(res.code==0){
                    for (let i = 0; i < res.data.length; i++) {
                        console.log(res)
                        var cate = res.data[i];
                        $("#category").append("<option value='"+cate.categoryId+"'>"+cate.categoryName+"</option>")
                    }
                }
            },
            "json"
        ...
        table.on("toolbar(goodsTable)",function (obj){
            switch (obj.event) {
                case "goodsAdd":
                    ...
                    $("#goodsForm")[0].reset();
                    //设置表单的提交地址
                    $("#actionUrl").val("${pageContext.request.contextPath}/Goods/add");
                    showGoodsForm();
                    break;
